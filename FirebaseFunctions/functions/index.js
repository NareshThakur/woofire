...
exports.dailySalesSummary = functions.pubsub.schedule('0 0 * * *').onRun(async (context) => {
  try {
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const eventsSnapshot = await admin.firestore().collection('events')
      .where('eventName', '==', 'purchased_item')
    ...
    const summary = `
      ...
      - Top Selling Products ID: ${topSellingProducts.join(', ')}
      - Total Revenue: ${ parseFloat(totalRevenue).toFixed(2)}
    `;
    
    // Send email
    await sendEmail('naresh.thakur1987@gmail.com', 'Daily Sales Summary', summary);
...
function calculateTotalRevenue(snapshot) {
  ...
  let totalRevenue = 0;
  snapshot.forEach((doc) => {
    const price = parseFloat(doc.data().total) || 0;
    totalRevenue += price;
  });
  return totalRevenue;
}
...
async function sendEmail(to, subject, text) {
  ...
  const mailOptions = {
    from: 'naresh.thakur1987@gmail.com',
    to: to,
    subject: subject,
    text: text,
  };
